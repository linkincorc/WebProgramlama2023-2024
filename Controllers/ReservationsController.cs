...
namespace PlaneTicket.Controllers
{
    ...
    public class ReservationsController : Controller
    {
        ...
        public IActionResult Create()
        {
            ...
            var upcomingFlights = _context.Flights
                .Where(f => f.DepartureTime > DateTime.UtcNow)
                .Select(f => new { f.Id, FlightDetails = f.FlightNumber + " (" + f.Origin + "-" + f.Destination + ")" });

            ViewData["FlightId"] = new SelectList(upcomingFlights, "Id", "FlightDetails");
        ...
        public async Task<IActionResult> Create([Bind("FlightId,SeatId")] Reservation reservation)
        {
            ...
            if (flight == null || seat == null)
            {
                ...
                ModelState.AddModelError("", "Flight or Seat not found.");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,FlightId,SeatId")] Reservation reservation)
        {
            ...
            try
            {
                ...
                if (existingReservation.SeatId != reservation.SeatId)
                {
                    ...
                    if (newSeat == null || !newSeat.IsAvailable)
                    {
                        ...
                        ViewData["FlightId"] =
                            new SelectList(_context.Flights, "Id", "FlightNumber", reservation.FlightId);
                        ViewData["SeatId"] = new SelectList(_context.Seats.Where(s => s.IsAvailable), "Id", "SeatCode",
                            reservation.SeatId);
                        return View(reservation);
                    }
